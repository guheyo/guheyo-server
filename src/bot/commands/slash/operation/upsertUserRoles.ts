...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      chunks.map((chunk, i) => {
        setTimeout(() => {
          chunk.map(async (memberAndRoleIds) => {
            try {
              const user = await getOrCreateUser(memberAndRoleIds.member);
              return await userService.setRoles(user.id, memberAndRoleIds.roleIds);
            } catch (e) {
              console.log(e);
            }
          });
